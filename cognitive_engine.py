# This project from Pulsar AI LABS TM -- is under the APACHE License. 
# Copyright 2025 The Pulsar AI authors. All Rights Reserved. 
...
"""
This file is the "brain, and main cognitive reasoning task orchestrator" It contains the core agent class and its
...
class UnifiedKnowledgeGraph:
    """
    A structured, graph-based model of the agent's reality. This is the
    foundation of the "Symbolic Mind" and replaces the unstructured LTM dict.
    """
    def __init__(self, agent_ref):
        self.agent: 'SimplifiedOrchOREmulator' = agent_ref
        self.G = nx.MultiDiGraph()

    def _get_node_name(self, type, identifier):
        """Standardizes node names for consistency."""
        return f"{type.upper()}:{identifier}"

    def ground_concept(self, concept_name: str, latent_vector: np.ndarray):
        """Adds or updates a 'concept' node, grounding it in sensory experience."""
        node_name = self._get_node_name("concept", concept_name)
        if node_name in self.G:
            # Update existing concept with a moving average for stability
            self.G.nodes[node_name]['latent_vector'] = \
                (self.G.nodes[node_name]['latent_vector'] * 0.9) + (latent_vector * 0.1)
            self.G.nodes[node_name]['count'] += 1
            self.G.nodes[node_name]['last_seen_cycle'] = self.agent.current_cycle_num
        else:
            self.G.add_node(node_name,
                type='concept',
                identifier=concept_name,
                latent_vector=latent_vector,
                count=1,
                first_seen_cycle=self.agent.current_cycle_num,
                last_seen_cycle=self.agent.current_cycle_num
            )
            self.agent._log_lot_event("ukg", "concept_grounded", {"concept": concept_name})
            if self.agent.verbose >= 1: print(f"  UKG: Grounded new concept '{concept_name}'.")

    def add_causal_link(self, action: int, preconditions: Dict, outcome: Dict):
        """
        The core learning function. Adds nodes and edges representing the statement:
        "Performing this ACTION with these PRECONDITIONS causes this OUTCOME."
        """
        # Ensure precondition state node exists
        precon_str = "state:" + "&".join(f"{k}={v}" for k, v in sorted(preconditions.items())) if preconditions else "state:none"
        if precon_str not in self.G:
            self.G.add_node(precon_str, type='state', identifier=precon_str)

        # Ensure action node exists
        action_str = f"action:{action}"
        if action_str not in self.G:
            self.G.add_node(action_str, type='action', identifier=action)

        # Link precondition state to the action
        self.G.add_edge(precon_str, action_str, type='IS_PRECONDITION')

        # Link action to the outcome concept
        interacted_concept = outcome.get('concept_interacted')
        if interacted_concept:
            outcome_concept_str = self._get_node_name("concept", interacted_concept)
            if outcome_concept_str not in self.G:
                 # If we interact with a concept we haven't seen, create a placeholder
                 self.G.add_node(outcome_concept_str, type='concept', identifier=interacted_concept, latent_vector=None)
            
            reward_val = outcome.get('reward', 0.0)
            # Strengthen edge based on reward
            edge_key = self.G.add_edge(action_str, outcome_concept_str, type='CAUSES_EFFECT', reward=reward_val)
            
            # Update edge confidence
            if self.G.has_edge(action_str, outcome_concept_str, key=edge_key):
                edge_data = self.G.get_edge_data(action_str, outcome_concept_str, key=edge_key)
                count = edge_data.get('count', 0) + 1
                total_reward = edge_data.get('total_reward', 0) + reward_val
                avg_reward = total_reward / count
                nx.set_edge_attributes(self.G, {(action_str, outcome_concept_str, edge_key): {'count': count, 'total_reward': total_reward, 'avg_reward': avg_reward}})

        self.agent._log_lot_event("ukg", "causal_link_added", {"action": action, "precon": precon_str, "outcome_concept": interacted_concept})

    def find_plan_for_goal(self, goal: GoalState) -> Optional[List[Dict]]:
        """
        Performs a backward-chaining search through the UKG to generate a plan.
        If a detailed plan cannot be found, it gracefully falls back to a simple, optimistic one.
        """
        # This planner is currently hardcoded for the 'main quest' as per the SE-³ roadmap.
        # It looks for the concept "target" as the ultimate goal.
        target_concept_name = "target"
        target_node = self._get_node_name("concept", target_concept_name)
        
        # **GRACEFUL FAILURE 1**: If the ultimate goal concept isn't even known yet,
        # we can't do any complex planning. We must fall back to a simple plan.
        if not self.G.has_node(target_node):
            if self.agent.verbose >= 1: print("UKG PLANNER: Cannot form a detailed plan; final 'target' concept is unknown. Falling back to simple plan.")
            return [{"name": "Achieve Final Goal", "target_concept": "target"}]

        # This is a simplified BFS backward search. A production system would use A* or similar.
        q = collections.deque([(target_node, [target_node])]) # (current_node, path_to_here)
        visited = {target_node}
        
        while q:
            current_concept, path = q.popleft()
            
            # Find actions that cause this concept
            predecessors = list(self.G.predecessors(current_concept))
            for action_node in predecessors:
                edge_data = self.G.get_edge_data(action_node, current_concept)
                # Check if it's a CAUSES_EFFECT link with positive reinforcement
                if any(data.get('type') == 'CAUSES_EFFECT' and data.get('avg_reward', -1) > 0 for key, data in edge_data.items()):
                    # Now find the preconditions for this effective action
                    action_predecessors = list(self.G.predecessors(action_node))
                    for precon_node in action_predecessors:
                        if self.G.nodes[precon_node]['type'] == 'state' and 'has_key=True' in precon_node:
                            # We found a causal link related to needing a key!
                            # In a real scenario, this would be a deeper recursive search.
                            if self.agent.verbose >= 1: print("UKG PLANNER: Found a causal chain requiring 'key'. Building multi-step plan.")

                            # Manually construct the full plan based on this insight.
                            # This simulates a successful recursive search for this specific problem.
                            key_step = {"name": "Acquire Key", "target_concept": "key"}
                            # To be robust, find the concept caused by the action that needs the key
                            door_concept_name = self.G.nodes[current_concept]['identifier']
                            door_step = {"name": f"Solve {door_concept_name} Obstacle", "target_concept": door_concept_name}
                            final_step = {"name": "Achieve Final Goal", "target_concept": "target"}
                            return [key_step, door_step, final_step]
                        
        # **GRACEFUL FAILURE 2**: If the loop finishes without finding a complex
        # causal chain, fall back to the simple, direct plan.
        if self.agent.verbose >= 1: print("UKG PLANNER: No complex causal chain found. Falling back to simple plan.")
        return [{"name": "Achieve Final Goal", "target_concept": "target"}]
        
    def to_dict(self):
        """Serializes graph to a JSON-friendly format for saving."""
        # Create a deep copy of the graph to avoid modifying the live graph in place.
        graph_copy = copy.deepcopy(self.G)
        # Convert numpy arrays to lists for JSON serialization on the copy.
        for n, data in graph_copy.nodes(data=True):
            if 'latent_vector' in data and isinstance(data['latent_vector'], np.ndarray):
                data['latent_vector'] = data['latent_vector'].tolist()
        return nx.node_link_data(graph_copy)

    def from_dict(self, data):
        """Deserializes graph from a dictionary."""
        # This function might fail if the data is not a valid graph structure
        try:
            # CORRECTED: Use nx.node_link_graph to correctly load a graph from dict.
            self.G = nx.node_link_graph(data)
            # CRITICAL FIX: Convert latent vector lists back to numpy arrays after loading
            for n, d in self.G.nodes(data=True):
                if 'latent_vector' in d and isinstance(d.get('latent_vector'), list):
                    d['latent_vector'] = np.array(d['latent_vector'])
        except Exception as e:
            # If loading fails (e.g., old/corrupt format), start with a fresh graph
            print(f"  UKG WARNING: Could not deserialize knowledge graph from save file ({e}). Starting fresh.")
            self.G = nx.MultiDiGraph()
# ---------------------------------------------------------------------------
...
# NEW
    def _update_goals_from_world_state(self):
        ...
        # --- NEW FOR SE-³: Autonomous Main Quest Generation ---
        # If the agent has discovered the "ultimate reward" state ('target') and doesn't
        # have an overarching life purpose, it creates one for itself.
        main_quest_exists = any(g.goal_type == 'MAIN_QUEST' for g in self.goals)
        target_node_name = self.agent.ukg._get_node_name("concept", "target")
        if self.agent.ukg.G.has_node(target_node_name) and not main_quest_exists:
            self.agent._log_lot_event("goal_manager", "main_quest_generated", {"reason": "discovered_target_concept"})
            if self.agent.verbose >= 1: print(f"[{self.agent.agent_id}] GOAL_MANAGER: Agent has discovered the 'target'. Generating its own Main Quest.")
            self.add_goal(GoalState(
                current_goal="Reach the final target", # This name is special for the planner
                goal_type="MAIN_QUEST",
                base_priority=0.7,
                steps=[], # Starts with an empty plan, forcing a PLAN_FORMULATION goal
                evaluation_criteria="GOAL_COMPLETION"
            ))
...
class SimplifiedOrchOREmulator:
    ...
    def __init__(self, agent_id="agent0", cycle_history_max_len=100,
        ...
        # --- NEW FOR SE-³ STAGE 3: Unified Knowledge Graph ---
        # The UKG replaces the old LTM dictionary and known_concept_vectors.
        # It's not shared between agents; each agent builds its own world model.
        self.ukg = UnifiedKnowledgeGraph(agent_ref=self)
        self.shared_attention_foci = shared_attention_foci if shared_attention_foci is not None else collections.deque(maxlen=20)

        # DEPRECATED LTM properties, kept for possible compatibility in minor functions.
        # The primary logic for LTM is now in the UKG.
        self.long_term_memory = {} # Kept as a shell for any old functions expecting it
...
### NEW VERSION ###
    ...
    def _executive_evaluate_outcome_and_update_mood(self, logical_outcome, orp_at_collapse, entropy_at_collapse, num_ops_executed_this_cycle):
        ...
        if self.internal_state_parameters['exploration_mode_countdown'] > 0:
            ...
            if self.verbose >= 2 and self.internal_state_parameters['exploration_mode_countdown'] == 0: # bla bla
    ...
    def _process_environmental_feedback(self, reward: float, terminated: bool, info: Dict):
        ...
        This now populates the Unified Knowledge Graph (UKG).
        """
        ...
        # --- Step 1: Ground Sensory Concepts in the UKG ---
        interacted_concept = info.get('interacted_with')
        if interacted_concept and self.last_perceived_state:
            self.ukg.ground_concept(interacted_concept, self.last_perceived_state.latent_vector)

        # --- Step 2: Learn Causal Links in the UKG ---
        ...
        if last_history_entry and last_history_entry.get('action_taken') is not None:
            ...
            # Add this experience as a causal link in the agent's world model
            self.ukg.add_causal_link(action_taken, preconditions, outcome)

        # --- Step 3: Learn Goal-State Vectors (for low-level planner) ---
        # This is still needed for the 'shooting planner' to aim at targets.
    ...
    def _executive_update_goal_progress(self, goal_to_evaluate: Optional[GoalState], current_state_handle: StateHandle):
        ...
        This version uses the UKG for concept checking and an exclusive criteria structure.
        """
        ...
        if goal.goal_type == 'SURVIVAL':
            ...
            elif target_concept_name:
                concept_node_name = self.ukg._get_node_name("concept", target_concept_name)
                if self.ukg.G.has_node(concept_node_name):
                    node_data = self.ukg.G.nodes[concept_node_name]
                    target_latent_vector = node_data.get('latent_vector')
        ...
        if step_completed:
            ...
            if goal.current_step_index >= len(goal.steps):
                ...
                # A MAIN_QUEST is persistent; it should remain in a 'completed' state.
    ...
    def _reasoning_tool_physical_shooting_planner(self, active_goal: GoalState) -> Optional[List[int]]:
        ...
        to reach the target concept of the active goal's current step. This now
        queries the Unified Knowledge Graph for target vectors.
        """
        ...
        if 0 <= active_goal.current_step_index < len(active_goal.steps):
            ...
            if target_concept_name:
                # The shooting planner needs a concrete vector to aim at. It gets this
                # from the grounded 'concept' node in the UKG.
                concept_node_name = self.ukg._get_node_name("concept", target_concept_name)
                if self.ukg.G.has_node(concept_node_name):
                     target_latent_vector = self.ukg.G.nodes[concept_node_name].get('latent_vector')
                else:
                     target_latent_vector = None
        ...
        if target_latent_vector is None:
            self._log_lot_event("reasoning_tool", "shooting_planner_fail", {"reason": "target_concept_not_grounded_in_ukg", "concept_name": target_concept_name})
            return None
...
# NEW
    def _reasoning_decompose_task(self, active_goal: GoalState) -> Optional[List[Dict]]:
        """
        Autonomous task decomposition, now driven by the Unified Knowledge Graph.
        """
        self._log_lot_event("planner", "task_decomposition_start", {"goal": active_goal.current_goal})

        generated_steps = self.ukg.find_plan_for_goal(active_goal)

        if generated_steps:
            if self.verbose >= 1: print(f"[{self.agent_id}] UKG PLANNER: Generated plan for '{active_goal.current_goal}': {[s['name'] for s in generated_steps]}")
            self._log_lot_event("planner", "decompose_success_ukg", {"steps": [s['name'] for s in generated_steps]})
        else:
             if self.verbose >= 1: print(f"[{self.agent_id}] UKG PLANNER: Failed to find a plan for '{active_goal.current_goal}'.")
             self._log_lot_event("planner", "decompose_fail_ukg", {})
...
# NEW
        # The `main_quest` check is now replaced by a generalized check on the active goal
        # This is the core of the Frustration-Driven Re-Planning loop
        # A goal is considered "plannable" and subject to frustration re-planning if it is a MAIN_QUEST.
        # This can be expanded to other goal_types in the future.
        if active_goal and active_goal.goal_type == 'MAIN_QUEST' and self.cycles_stuck_on_step > 40:
            if self.verbose >= 1: print(f"[{self.agent_id}] High frustration on Main Quest! Re-evaluating plan for '{active_goal.current_goal}'.")
            self._log_lot_event("planner", "replan_trigger", {"reason": "stuck_on_step", "goal": active_goal.current_goal, "stuck_cycles": self.cycles_stuck_on_step})
            active_goal.steps = []  # WIPE THE FAILED PLAN
            active_goal.current_step_index = 0
            active_goal.status = 'active' # Reset status in case it was failed
            active_goal.history.append({"cycle": self.current_cycle_num, "event": "frustration_replan_triggered"})

            self.cycles_stuck_on_step = 0
            self.internal_state_parameters['frustration'] = min(0.95, self.internal_state_parameters['frustration'] + 0.4)

            # Wiping steps will trigger PLAN_FORMULATION on the next cycle. For this cycle, we explore randomly.
            return 'PHYSICAL', [self.action_space.sample()]
    ...
    def load_state(self):
        ...
        if os.path.exists(filepath):
            ...
            try:
                ...
                if 'ukg' in state_data:
                    self.ukg.from_dict(state_data['ukg'])
                elif 'long_term_memory' in state_data: # Compatibility with older saves
                ...
                # 'known_concept_vectors' is deprecated; its data is now in the UKG.
                # We can try to load it for backwards compatibility if needed, but new agents won't use it.
                if 'known_concept_vectors' in state_data and not 'ukg' in state_data:
                     # Could add logic here to migrate old concepts to new UKG format
                     pass
